...
| Taste | Aktion |
|---|---|
| **M1** | Beim Loslassen: zweizeiliger TODO-Kommentar mit Name/Kürzel und aktuellem Zeitstempel, z. B. `// Ruemmler, Elias (Ehp/TEF1.3) 23.09.2026 10:23:34` gefolgt von einer leeren Kommentarzeile `// ` (Zeilenumbruch per Shift+Enter) |
...
| **M1 + M3** (gleichzeitig) | Gibt einen hinterlegten Text (`TEXT_M4`) + Enter aus – **nur** wenn entweder Cisco AnyConnect im Vordergrund ist oder der PC gerade gesperrt ist. `M2` bleibt davon unbeteiligt |
...
- `WIN:` bei jedem Wechsel der aktiven Anwendung (VS, VS Code, KiCad, Opera,
  Explorer, Outlook, GitHub Desktop, Teams, Cisco AnyConnect – alles andere
  als `DEFAULT`)
...
ersetzt, das nur bei aktivem AnyConnect bzw. gesperrtem PC getippt wird. Wer
